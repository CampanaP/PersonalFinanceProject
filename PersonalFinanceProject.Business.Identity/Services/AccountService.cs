...
namespace PersonalFinanceProject.Business.Account.Services
{
    ...
    internal class AccountService : IAccountService
    {
        ...
        public async Task Registration(IdentityUser user, string password, CancellationToken cancellationToken = default)
        {
            bool isSuccess = await _identityUserRepository.Create(user, password);
            if (!isSuccess)
            {
                throw new Exception($"{nameof(AccountService)} - {nameof(Registration)} - Create is in Error - User Email: {user.Email}");
            }
